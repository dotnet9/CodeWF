...
CodeWF 前台站点的 HTML 高保真原型。用浏览器直接打开 `mockups/*.html` 即可预览，无外部字体/脚本/图片依赖（图片以占位块表示）。也可以 `node mockups/.serve.js` 起本地服务器预览（默认 `http://127.0.0.1:8137`），交互（Ctrl K 面板、打字机等）体验一致。
...
- **`Ctrl K` 命令面板搜索**（`shared.js` 已实现，点击导航搜索框或按 `/` 同样唤起；索引页面/文章/工具/专题，支持 ↑↓ 选择、Enter 跳转、命中高亮）、卡片 hover 上浮泛紫色描边、终端光标闪烁、时间轴发光节点。
...
| `mockups/album-detail.html` | `/album/[slug]` | 专题详情：头卡统计 + 按章节分组的学习路径（31 篇真实系列文章）、阅读进度、相关专题 |
...
| `mockups/404.html` | 任意未知路由 | 终端风错误页：`curl 404` 打字机 + glitch 大字 + 快捷出口 |

共享样式见 `mockups/shared.css`，全局交互见 `mockups/shared.js`（命令面板、导航滚动态）。多语言（中/EN/JA/繁）在导航右侧以切换器呈现，对应数据文件的 `*.en.json` / `*.ja.json` / `*.zh-tw.json` 后缀机制。

## 原型数据来源

原型中的文章标题、分类、专题、工具、时间线、友链均为真实数据，取自本地内容仓库 `E:\github\web\Assets.Dotnet9`：
...
- `Ctrl K` 命令面板在实现中对应全局搜索（`/search`）；`shared.js` 中的 `INDEX` 数组替换为 `/api/search` 的异步检索即可，键盘交互逻辑可原样迁移。
