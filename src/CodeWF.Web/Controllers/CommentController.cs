    ...
    ILogger<CommentController> logger) : ControllerBase
{
    ...
    public async Task<IActionResult> Create([NotEmpty] Guid postId, CommentRequest request)
    {
        ...
        string ip = (bool)HttpContext.Items["DNT"]! ? "N/A" : Helper.GetClientIP(HttpContext);
        (int Status, CommentDetailedItem Item)
            item = await mediator.Send(new CreateCommentCommand(postId, request, ip));
        ...
        {
            return Created("codewf://empty", item);
        }
        ...
        LinkGenerator linkGenerator)
    {
        ...
        CommentReply reply = await mediator.Send(new ReplyCommentCommand(commentId, replyContent));
        if (blogConfig.NotificationSettings.SendEmailOnCommentReply && !string.IsNullOrWhiteSpace(reply.Email))
        {
            string postLink = GetPostUrl(linkGenerator, reply.PubDateUtc, reply.Slug);
    ...
    {
        string? link = linkGenerator.GetUriByPage(HttpContext, "/Post", null,
            new { year = pubDate.Year, month = pubDate.Month, day = pubDate.Day, slug });
        return link;
    }
}