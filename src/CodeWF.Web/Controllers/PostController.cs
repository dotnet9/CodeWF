    ...
    ILogger<PostController> logger) : ControllerBase
{
    ...
    public async Task<IActionResult> CreateOrEdit(PostEditModel model, LinkGenerator linkGenerator)
    {
        try
        {
            ...
            DateTime tzDate = timeZoneResolver.NowOfTimeZone;
            ...
            PostEntity postEntity = model.PostId == Guid.Empty
            ...
            if (model.IsPublished)
            {
                ...
                DateTime pubDate = postEntity.PubDateUtc.GetValueOrDefault();

                string? link = linkGenerator.GetUriByPage(HttpContext, "/Post", null,
                    new { year = pubDate.Year, month = pubDate.Month, day = pubDate.Day, postEntity.Slug });
    ...
    [TypeFilter(typeof(ClearBlogCache),
        Arguments = new object[] { BlogCacheType.Subscription | BlogCacheType.SiteMap })]
    ...
    {
        return Ok(new { ServerTime = DateTime.UtcNow, Nonce = nonce });
    }
}