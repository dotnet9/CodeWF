    ...
    IMediator mediator) : ControllerBase
{
    ...
    public async Task<IActionResult> Image(ImageSettings model, IBlogImageStorage imageStorage)
    {
        ...
        if (model.EnableCDNRedirect)
        {
                ...
                && !blogConfig.GeneralSettings.AvatarUrl.StartsWith(model.CDNEndpoint))
            {
                try
                {
                    string avatarData = await mediator.Send(new GetAssetQuery(AssetId.AvatarBase64));

                    if (!string.IsNullOrWhiteSpace(avatarData))
                    {
                        byte[] avatarBytes = Convert.FromBase64String(avatarData);
                        string fileName = $"avatar-{AssetId.AvatarBase64:N}.png";
    ...
    {
        logger.LogWarning(
            $"System reset is requested by '{User.Identity?.Name}', IP: {Helper.GetClientIP(HttpContext)}.");
    ...
    public async Task<IActionResult> CustomStyleSheet(CustomStyleSheetSettings model)
    {
        ...
        UglifyResult uglifyTest = Uglify.Css(model.CssCode);
    ...
    public async Task<IActionResult> CustomMenu(CustomMenuSettingsJsonModel model)
    {
        ...
        blogConfig.CustomMenuSettings = new CustomMenuSettings
        {
            IsEnabled = model.IsEnabled, Menus = model.MenuJson.FromJson<Menu[]>()
        };
    ...
    {
        string? password = passwordGenerator.GeneratePassword(new PasswordRule(10, 3));
        return Ok(new { ServerTimeUtc = DateTime.UtcNow, Password = password });
    }

    private async Task SaveConfigAsync<T>(T blogSettings) where T : IBlogSettings
    {
        KeyValuePair<string, string> kvp = blogConfig.UpdateAsync(blogSettings);
