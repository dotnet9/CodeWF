    ...
    ILogger<AssetsController> logger) : ControllerBase
{
    ...
    {
        string fallbackImageFile = Path.Join($"{env.WebRootPath}", "images", "default-avatar.png");

        byte[]? bytes = await cache.GetOrCreateAsync(BlogCachePartition.General.ToString(), "avatar", async _ =>
        {
            logger.LogTrace("Avatar not on cache, getting new avatar image...");

            string data = await mediator.Send(new GetAssetQuery(AssetId.AvatarBase64));
            if (string.IsNullOrWhiteSpace(data))
            {
                return null;
            }

            byte[] avatarBytes = Convert.FromBase64String(data);
            return avatarBytes;
        });
    ...
    public async Task<IActionResult> Avatar([FromBody] string base64Img)
    {
        ...
        if (!Helper.TryParseBase64(base64Img, out byte[] base64Chars))
        {
        ...
        try
        {
            using Image bmp = await Image.LoadAsync(new MemoryStream(base64Chars));
    ...
    {
        byte[]? iconBytes = MemoryStreamIconGenerator.GetIcon(filename);
    ...
    public async Task<IActionResult> SiteIconOrigin()
    {
        ...
        try
        {
            byte[] siteIconBytes = Convert.FromBase64String(data);
    ...
    public async Task<IActionResult> UpdateSiteIcon([FromBody] string base64Img)
    {
        ...
        {
            return Conflict("image height must be equal to width");
        }
