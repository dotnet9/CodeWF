...
public class ThemeController(IMediator mediator, ICacheAside cache, IBlogConfig blogConfig) : ControllerBase
{
    ...
    public async Task<IActionResult> Css()
    {
        try
        {
            string? css = await cache.GetOrCreateAsync(BlogCachePartition.General.ToString(), "theme", async entry =>
            {
                ...
                string data = await mediator.Send(new GetSiteThemeStyleSheetQuery(blogConfig.GeneralSettings.ThemeId));
                return data;
            });
            ...
            UglifyResult uCss = Uglify.Css(css);
            if (uCss.HasErrors)
            {
    ...
    public async Task<IActionResult> Create(CreateThemeRequest request)
    {
        ...
        int id = await mediator.Send(new CreateThemeCommand(request.Name, dic));
        if (id == 0)
        {
    ...
    {
        OperationCode oc = await mediator.Send(new DeleteThemeCommand(id));
        return oc switch
        {
