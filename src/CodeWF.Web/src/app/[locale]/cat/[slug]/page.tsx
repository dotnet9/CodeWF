...
export default async function CategoryPage({ params, searchParams }: Props) {
  ...
  const category = (await api.categories(locale)).find((item) => item.slug?.toLowerCase() === categorySlug.toLowerCase());
  const filter = category?.name ?? category?.slug ?? categorySlug;
  const posts = await api.posts(locale, { pageIndex, pageSize: 12, category: filter });
  ...
  return (
    <main className="page-wrap">
      ...
      <Pagination locale={locale} pageIndex={posts.pageIndex} pageSize={posts.pageSize} total={posts.total} basePath={`/cat/${slug}`} />
    </main>
  );
}
