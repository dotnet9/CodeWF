using CodeWF.WebAPI.Domain.BlogPosts;
using System.Linq.Expressions;

namespace CodeWF.Web.Service.BlogPosts;

internal class BlogPostService : IBlogPostService
{
    private readonly CodeWFDbContext _dbContext;

    public BlogPostService(CodeWFDbContext dbContext)
    {
    ...
    private async Task<BlogPostDetails?> BlogPostDetailsByAsync(Expression<Func<BlogPost, bool>> predicate)
    {
        ...
        var near = await (from post in _dbContext.BlogPosts!
            ...
            where blogPostCategory.BlogPostId != blogPost.Id &&
                  categories.Select(x => x.Id).ToList().Contains(blogPostCategory.CategoryId)
