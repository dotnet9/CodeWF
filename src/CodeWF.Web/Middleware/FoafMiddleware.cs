...
public class FoafMiddleware(RequestDelegate next)
{
        ...
        LinkGenerator linkGenerator)
    {
        if (context.Request.Path == "/foaf.xml")
        {
            ...
            IReadOnlyList<FriendLinkEntity> friends = await mediator.Send(new GetAllLinksQuery());
            FoafDoc foafDoc = new FoafDoc
            {
            ...
            string requestUrl = GetUri(context.Request).ToString();
            string xml = await mediator.Send(new WriteFoafCommand(foafDoc, requestUrl, friends));
