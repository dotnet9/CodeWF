...
public class StyleSheetMiddleware(RequestDelegate next)
{
    ...
    public async Task Invoke(HttpContext context, IBlogConfig blogConfig, IMediator mediator)
    {
        ...
        if (context.Request.Path == Options.DefaultPath)
        {
            ...
            string cssCode = blogConfig.CustomStyleSheetSettings.CssCode;
            await WriteStyleSheet(context, cssCode);
        }
        else if (context.Request.Path == "/content.css" && context.Request.QueryString.HasValue)
        {
            // Get query string value
            NameValueCollection qs = HttpUtility.ParseQueryString(context.Request.QueryString.Value!);
            string? id = qs["id"];
            ...
            if (!string.IsNullOrWhiteSpace(id))
            {
                ...
                {
                    StyleSheetEntity? css = await mediator.Send(new GetStyleSheetQuery(guid));
                    if (css == null)
                    {
                        context.Response.StatusCode = StatusCodes.Status404NotFound;
                        return;
    ...
    private static async Task WriteStyleSheet(HttpContext context, string cssCode)
    {
        ...
        UglifyResult uglifiedCss = Uglify.Css(cssCode);
...
{
    public static IApplicationBuilder UseCustomCss(this IApplicationBuilder app,
        Action<CustomCssMiddlewareOptions> options)
    {
