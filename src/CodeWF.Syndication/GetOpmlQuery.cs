...
public class GetOpmlQueryHandler : IRequestHandler<GetOpmlQuery, string>
{
    public async Task<string> Handle(GetOpmlQuery request, CancellationToken ct)
    {
        ...
        await using (XmlWriter writer = XmlWriter.Create(sb, writerSettings))
        {
            ...
            // categories
            foreach (KeyValuePair<string, string> cat in request.OpmlDoc.ContentInfo)
            {
                // open OUTLINE
                writer.WriteStartElement("outline");
                ...
                writer.WriteAttributeString("type", "rss");
                writer.WriteAttributeString("xmlUrl",
                    request.OpmlDoc.XmlUrlTemplate.Replace("[catTitle]", cat.Value).ToLower());
                writer.WriteAttributeString("htmlUrl",
                    request.OpmlDoc.HtmlUrlTemplate.Replace("[catTitle]", cat.Value).ToLower());
