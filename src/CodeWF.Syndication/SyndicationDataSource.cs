...
public class SyndicationDataSource : ISyndicationDataSource
{
    ...
    public async Task<IReadOnlyList<FeedEntry>> GetFeedDataAsync(string catRoute = null)
    {
        ...
        {
            CategoryEntity? cat = await _catRepo.GetAsync(c => c.RouteName == catRoute);
            if (cat is null)
            {
                return null;
            }
    ...
    private async Task<IReadOnlyList<FeedEntry>> GetFeedEntriesAsync(Guid? catId = null)
    {
        ...
        PostSpec postSpec = new PostSpec(catId, top);
        IReadOnlyList<FeedEntry?> list = await _postRepo.SelectAsync(postSpec,
            p => p.PubDateUtc != null
                ? new FeedEntry
                {
                    Id = p.Id.ToString(),
                    ...
                    Link =
                        $"{_baseUrl}/post/{p.PubDateUtc.Value.Year}/{p.PubDateUtc.Value.Month}/{p.PubDateUtc.Value.Day}/{p.Slug}",
        ...
        {
            foreach (FeedEntry? simpleFeedItem in list)
            {
