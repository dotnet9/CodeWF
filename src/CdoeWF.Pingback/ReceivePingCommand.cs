    ...
    IRepository<PostEntity> postRepo) : IRequestHandler<ReceivePingCommand, PingbackResponse>
{
    ...
    public async Task<PingbackResponse> Handle(ReceivePingCommand request, CancellationToken ct)
    {
        try
        {
            ...
            bool valid = ValidateRequest(request.RequestBody);
            if (!valid)
            {
            ...
            PingRequest? pingRequest = await pingSourceInspector.ExamineSourceAsync(_sourceUrl, _targetUrl);
            if (null == pingRequest)
            {
                return PingbackResponse.InvalidPingRequest;
            }
            ...
            (string slug, DateTime pubDate) = GetSlugInfoFromUrl(pingRequest.TargetUrl);
            PostSpec spec = new PostSpec(pubDate, slug);
            (Guid id, string title) =
                await postRepo.FirstOrDefaultAsync(spec, p => new Tuple<Guid, string>(p.Id, p.Title));
            if (id == Guid.Empty)
            {
            ...
            bool pinged = await pingbackRepo.AnyAsync(p =>
                p.TargetPostId == id &&
            ...
            PingbackEntity obj = new PingbackEntity
            {
                Id = Guid.NewGuid(),
    ...
    {
        Regex blogSlugRegex =
            new Regex(@"^https?:\/\/.*\/post\/(?<yyyy>\d{4})\/(?<MM>\d{1,12})\/(?<dd>\d{1,31})\/(?<slug>.*)$");
