...
public partial class SeedService : ISeedService
{
    ...
    public SeedService(CodeWFDbContext dbContext, IOptions<SiteOptions> siteOptions, IIdRepository iDRepository,
    ...
    {
        Console.WriteLine($"1、创建数据库表结构：数据库如果不存在，EnsureCreatedAsync会根据{nameof(CodeWFDbContext)}配置创建");
        await CreateDatabaseAsync();
        Console.WriteLine("2、创建默认用户");
        await CreateDefaultUser();
        Console.WriteLine(
            $"3、添加种子数据，需要先将数据从 https://github.com/CodeWF/Assets.CodeWF 克隆，并配置到 {_siteOptions.AssetsLocalPath}");
    ...
    private async Task CreateDefaultUser()
    {
        ...
        IdentityResult r = await _iDRepository.CreateAsync(user, "CodeWF");
