...
namespace CodeWF.Data.Infrastructure;
    ...
    where T : class
{
    ...
    {
        return DbContext.Set<T>().FindAsync(new[] { key }, ct);
    }
    ...
    {
        T? entity = await GetAsync(key, ct);
    ...
    {
        return await ApplySpecification(spec).AsNoTracking().Select(selector).ToListAsync(ct);
    }

    public Task<TResult?> FirstOrDefaultAsync<TResult>(ISpecification<T>? spec, Expression<Func<T, TResult>> selector)
    {
