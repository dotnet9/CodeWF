...
public class CreateCommentCommand(Guid postId, CommentRequest payload, string ipAddress)
    : IRequest<(int Status, CommentDetailedItem Item)>
{
    ...
    IRequestHandler<CreateCommentCommand, (int Status, CommentDetailedItem Item)>
{
    public async Task<(int Status, CommentDetailedItem Item)> Handle(CreateCommentCommand request, CancellationToken ct)
    {
        ...
        PostSpec spec = new PostSpec(request.PostId, false);
        var postInfo = await postRepo.FirstOrDefaultAsync(spec, p => new { p.Title, p.PubDateUtc });

        if (blogConfig.ContentSettings.CloseCommentAfterDays > 0)
        {
            int days = DateTime.UtcNow.Date.Subtract(postInfo.PubDateUtc.GetValueOrDefault()).Days;
        ...
        CommentDetailedItem item = new CommentDetailedItem
        {
            Id = model.Id,
