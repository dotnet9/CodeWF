...
using AntDesign;
using CodeWF.Shared.Utils;
using AntDesign.Extensions.Localization;
using Microsoft.AspNetCore.Components;
using Microsoft.AspNetCore.Components.Rendering;

namespace CodeWF.Shared.Services
{
    public class DemoService
    {
        ...
        private void Initialize(string language)
        {
            ...
            {
                var menuItems = await _httpClient.GetFromJsonAsync<DemoMenuItem[]>(new Uri(_baseUrl, $"_content/CodeWF.Shared/meta/menu.{language}.json").ToString());
                return menuItems;
            }));
            ...
            {
                var components = await _httpClient.GetFromJsonAsync<DemoComponent[]>(new Uri(_baseUrl, $"_content/CodeWF.Shared/meta/components.{language}.json").ToString());
            ...
            {
                var menuItems = await _httpClient.GetFromJsonAsync<DemoMenuItem[]>(new Uri(_baseUrl, $"_content/CodeWF.Shared/meta/demos.{language}.json").ToString());
                return menuItems;
            }));
            ...
            {
                var menuItems = await _httpClient.GetFromJsonAsync<DemoMenuItem[]>(new Uri(_baseUrl, $"_content/CodeWF.Shared/meta/docs.{language}.json").ToString());
                return menuItems;
            }));
        }
        ...
        //    var demoTypes = await _httpClient.GetFromJsonAsync<string[]>(new Uri(_baseUrl, $"_content/CodeWF.Shared/meta/demoTypes.json").ToString());
