    ...
    public partial class Components : ComponentBase, IDisposable
    {
        ...
        private async Task HandleNavigate()
        {
            ...
            else
            {
                ...
                _filePath = $"site/CodeWF.Shared/Demos/Components/{_demoComponent?.Title}/doc/index.{CurrentLanguage}.md";
                _filePaths = new() { _filePath };
                foreach (var item in _demoComponent.DemoList?.Where(x => !x.Debug && !x.Docs.HasValue) ?? Array.Empty<DemoItem>())
                {
                    _filePaths.Add($"site/CodeWF.Shared/Demos/Components/{_demoComponent?.Title}/demo/{item.Name}.md");
                    _filePaths.Add($"site/CodeWF.Shared/{item.Type.Replace(".", "/")}.razor");
                }
                StateHasChanged();
            }
        }
