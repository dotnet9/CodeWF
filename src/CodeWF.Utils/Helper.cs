...
public static class Helper
{
    public static string AppVersion
    {
        get
        {
            ...
            {
                string gitHash = version[(version.IndexOf('+') + 1)..]; // e57ab0321ae44bd778c117646273a77123b6983f
                string prefix = version[..version.IndexOf('+')]; // 11.2-preview
    ...
    public static string TryGetFullOSVersion()
    {
        ...
        try
        {
            ...
            {
                object name = currentVersion.GetValue("ProductName", "Microsoft Windows NT");
                string? ubr = currentVersion.GetValue("UBR", string.Empty)?.ToString();
    ...
    public static string ResolveRootUrl(HttpContext ctx, string canonicalPrefix, bool preferCanonical = false,
        bool removeTailSlash = false)
    {
        if (ctx is null && !preferCanonical)
        {
            throw new ArgumentNullException(nameof(ctx),
                "HttpContext must not be null when preferCanonical is 'false'");
        }

        string url = preferCanonical
    ...
    {
        return IPAddress.Parse(ip).GetAddressBytes() switch
        {
    ...
    public static string FormatCopyright2Html(string copyrightCode)
    {
        ...
        string result = copyrightCode.Replace("[c]", "&copy;")
    ...
    {
        IEnumerable<string> messages = GetErrorMessagesFromModelState(modelStateDictionary);
        string[] enumerable = messages as string[] ?? messages.ToArray();
