...
public class DistributedCacheHelper : IDistributedCacheHelper
{
    ...
    {
        var jsonStr = distCache.GetString(cacheKey);
        //缓存中不存在
        if (string.IsNullOrEmpty(jsonStr))
        {
            var options = CreateOptions(expireSeconds);
            var result = valueFactory(options); //如果数据源中也没有查到，可能会返回null
            //null会被json序列化为字符串"null"，所以可以防范“缓存穿透”
            var jsonOfResult = JsonSerializer.Serialize(result,
                typeof(TResult));
    ...
    {
        var jsonStr = await distCache.GetStringAsync(cacheKey);
        if (string.IsNullOrEmpty(jsonStr))
        {
            var options = CreateOptions(expireSeconds);
    ...
    private static DistributedCacheEntryOptions CreateOptions(int baseExpireSeconds)
    {
        ...
        var sec = Random.Shared.NextDouble(baseExpireSeconds, baseExpireSeconds * 2);
