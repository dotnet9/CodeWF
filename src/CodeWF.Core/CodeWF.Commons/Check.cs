...
public static class Check
{
    ...
    public static string NotNull(string? value, string parameterName, int maxLength = int.MaxValue, int minLength = 0)
    {
        if (value == null)
        {
            throw new ArgumentException(parameterName + " 不能为null!", parameterName);
        }
        ...
        int minLength = 0)
    {
        ...
        {
            throw new ArgumentException(parameterName + " 不能为null，空，空白字符!", parameterName);
        }
        ...
        int minLength = 0)
    {
        ...
        {
            throw new ArgumentException(parameterName + " 不能为null或者空!", parameterName);
        }
