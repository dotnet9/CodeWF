...
public class UpdatePageCommandHandler(IRepository<PageEntity> repo, IMediator mediator)
    : IRequestHandler<UpdatePageCommand, Guid>
{
    public async Task<Guid> Handle(UpdatePageCommand request, CancellationToken ct)
    {
        (Guid guid, EditPageRequest payload) = request;
        PageEntity? page = await repo.GetAsync(guid, ct);
        if (page is null)
        {
