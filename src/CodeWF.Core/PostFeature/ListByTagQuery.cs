    ...
    : IRequestHandler<ListByTagQuery, IReadOnlyList<PostDigest>>
{
    public Task<IReadOnlyList<PostDigest>> Handle(ListByTagQuery request, CancellationToken ct)
    {
        if (request.TagId <= 0)
        {
            throw new ArgumentOutOfRangeException(nameof(request.TagId));
        }

        Helper.ValidatePagingParameters(request.PageSize, request.PageIndex);

        Task<IReadOnlyList<PostDigest>> posts = repo.SelectAsync(
            new PostTagSpec(request.TagId, request.PageSize, request.PageIndex), PostDigest.EntitySelectorByTag, ct);
        return posts;
    }
}