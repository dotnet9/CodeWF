...
{
    public static readonly Expression<Func<PostTagEntity, PostDigest>> EntitySelectorByTag = p => new PostDigest
    {
        Title = p.Post.Title,
        Slug = p.Post.Slug,
        ...
        Tags = p.Post.Tags.Select(
            pt => new Tag { NormalizedName = pt.NormalizedName, DisplayName = pt.DisplayName })
    };
    ...
    public static Expression<Func<PostEntity, PostDigest>> EntitySelector => p => new PostDigest
    {
        ...
        Tags = p.Tags.Select(pt => new Tag { NormalizedName = pt.NormalizedName, DisplayName = pt.DisplayName })
    };
}