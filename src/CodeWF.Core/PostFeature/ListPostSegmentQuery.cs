...
{
    public async Task<(IReadOnlyList<PostSegment> Posts, int TotalRows)> Handle(ListPostSegmentQuery request,
        CancellationToken ct)
    {
        ...
        PostPagingSpec spec = new PostPagingSpec(request.PostStatus, request.Keyword, request.PageSize, request.Offset);
        IReadOnlyList<PostSegment> posts = await repo.SelectAsync(spec, PostSegment.EntitySelector, ct);
        ...
        int totalRows = await repo.CountAsync(countExp, ct);
