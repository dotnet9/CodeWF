    ...
    public async Task<int> Handle(CountPostQuery request, CancellationToken ct)
    {
        ...
        switch (request.CountType)
        {
            ...
            case CountType.Category:
                ...
                {
                    throw new ArgumentNullException(nameof(request.CatId));
                }
            ...
            case CountType.Tag:
                ...
                {
                    throw new ArgumentNullException(nameof(request.TagId));
                }

                count = await postTagRepo.CountAsync(
                    p => p.TagId == request.TagId.Value && p.Post.IsPublished && !p.Post.IsDeleted, ct);
                break;
