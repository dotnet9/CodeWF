...
public class CreateCategoryCommandHandler(IRepository<CategoryEntity> catRepo, ICacheAside cache)
    : IRequestHandler<CreateCategoryCommand>
{
    public async Task Handle(CreateCategoryCommand request, CancellationToken ct)
    {
        bool exists = await catRepo.AnyAsync(c => c.RouteName == request.RouteName, ct);
        if (exists)
        {
            return;
        }
