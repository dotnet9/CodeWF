...
public class SaveAssetCommandHandler(IRepository<BlogAssetEntity> repo) : INotificationHandler<SaveAssetCommand>
{
    public async Task Handle(SaveAssetCommand request, CancellationToken ct)
    {
        ...
        {
            throw new ArgumentOutOfRangeException(nameof(request.AssetId));
        }

        if (string.IsNullOrWhiteSpace(request.AssetBase64))
        {
            throw new ArgumentNullException(nameof(request.AssetBase64));
        }

        BlogAssetEntity? entity = await repo.GetAsync(request.AssetId, ct);
        ...
        if (null == entity)
        {
            await repo.AddAsync(
                new BlogAssetEntity
                {
                    Id = request.AssetId, Base64Data = request.AssetBase64, LastModifiedTimeUtc = DateTime.UtcNow
                }, ct);
        }
        else
        {
